...
CREATE VIEW Friends
AS SELECT initiator_user_id, target_user_id
FROM friend_requests f JOIN users u 
WHERE f.initiator_user_id = u.id AND initiator_user_id = 1 ; 
...
SELECT * FROM ListMedia

-- 4. Получите количество групп у пользователей

CREATE VIEW GroupUsers
AS SELECT user_id, community_id,
		COUNT(uc.community_id) AS total_group
FROM users_communities uc JOIN communities c 
...
SELECT firstname, lastname, count_message, DENSE_RANK() OVER (ORDER BY count_message DESC) AS ranking
FROM (
	users.firstname, users.lastname, count_message
		FROM (
		SELECT from_users_id, count(*) count_message
        FROM messages
        GROUP BY from_users_id
        ) AS user_count
	JOIN users ON user_count.from_user_id = users.id
	ORDER BY count_message DESC
) AS ranked_users;
       

-- 7. Выберите все сообщения, отсортируйте сообщения по возрастанию даты отправления (created_at) 
-- и найдите разницу дат отправления между соседними сообщениями, получившегося списка.

CREATE VIEW SortMessage
AS SELECT *
FROM messages
...
SELECT
    ...
    body,
    created_at,
    LEAD(created_at) OVER(ORDER BY created_at ASC) - created_at AS date_difference
FROM SortMessage;
